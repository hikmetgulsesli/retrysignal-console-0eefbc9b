...
- RetrySignal Console turns the user's request into a directly usable web workflow. The first experience must be the actual web product behavior, not a marketing landing page or placeholder demo.
- - FR-001: Build a compact browser operations app called RetrySignal Console. It should be a Vite React app for monitoring retry recoveries, PR review feedback, supervisor decisions, stale review comments, story status, and final gate outcomes. Include status filters, editable local notes, a recovery timeline, persiste…
...
- Feedback Operations - RetrySignal Console
- Feedback Editor - RetrySignal Console
- Pipeline Board - RetrySignal Console
- Settings and Preferences - RetrySignal Console
- Empty and Error Recovery - RetrySignal Console

## SCREEN_SPECS
SCREEN_SPEC_1:
- exact_screen_title: Feedback Operations - RetrySignal Console
- surface_id: SURF_FEEDBACK_OPERATIONS
- unique_canvas_caption: Feedback Operations: summary metrics, primary list/board/table, filters, search, selected item preview, empty/loading/error states.
- purpose: Give the user the main operational view for inspecting, searching, filtering, and acting on Feedback data.
...
- data_entities: Feedback, ActivityEvent, Preference
...
SCREEN_SPEC_2:
- exact_screen_title: Feedback Editor - RetrySignal Console
- surface_id: SURF_FEEDBACK_EDITOR
- unique_canvas_caption: Feedback Editor: form fields, required/optional indicators, validation messages, save/cancel controls, unsaved-state feedback.
- purpose: Let the user create, edit, validate, save, cancel, and recover Feedback changes.
...
- data_entities: Feedback, ValidationError
- visible_actions: ACT_SAVE_RECORD as form_submit, ACT_CANCEL_EDIT as secondary_button
- entry_exit_rules: SURF_FEEDBACK_OPERATIONS -> Save returns to SURF_FEEDBACK_OPERATIONS with persisted changes; cancel preserves existing data and closes the editor.
- design_guidance: Form layout must be clear and task-specific; do not invent payment, onboarding, or unrelated identity forms.

SCREEN_SPEC_3:
- exact_screen_title: Pipeline Board - RetrySignal Console
...
- purpose: Help users organize Feedback work by pipeline stage and board state when those signals are part of the requested product.
...
- entry_exit_rules: SURF_FEEDBACK_OPERATIONS -> Status changes keep the user in the same Feedback context and expose recoverable failure feedback.
- design_guidance: Represent board state clearly; do not introduce service-desk queues or unrelated analytics modules.

SCREEN_SPEC_4:
- exact_screen_title: Settings and Preferences - RetrySignal Console
...
- purpose: Let users adjust Feedback workflow preferences, saved filters, defaults, or product settings requested by the task.
...
- entry_exit_rules: SURF_FEEDBACK_OPERATIONS -> Saved preferences immediately affect visible product state or show a clear confirmation.
- design_guidance: Settings must support the requested workflow only; do not invent unrelated profile or billing areas.

SCREEN_SPEC_5:
- exact_screen_title: Empty and Error Recovery - RetrySignal Console
...
- purpose: Keep the Feedback workflow usable when data is missing, filtered away, loading, failed, or corrupt.
...
- entry_exit_rules: SURF_FEEDBACK_OPERATIONS, SURF_INSIGHTS -> Recovery returns to the active Feedback workflow and preserves unrelated state.
...
## PRODUCT_SURFACES
1. SURF_FEEDBACK_OPERATIONS - Feedback Operations
   Purpose: Give the user the main operational view for inspecting, searching, filtering, and acting on Feedback data.
   Data: Feedback, ActivityEvent, Preference
...
2. SURF_FEEDBACK_EDITOR - Feedback Editor
   Purpose: Let the user create, edit, validate, save, cancel, and recover Feedback changes.
   Data: Feedback, ValidationError
   ...
   Entry/exit: SURF_FEEDBACK_OPERATIONS -> Save returns to SURF_FEEDBACK_OPERATIONS with persisted changes; cancel preserves existing data and closes the editor.
...
3. SURF_PIPELINE_BOARD - Pipeline Board
   ...
   Entry/exit: SURF_FEEDBACK_OPERATIONS -> Status changes keep the user in the same Feedback context and expose recoverable failure feedback.
...
4. SURF_SETTINGS_AND_PREFERENCES - Settings and Preferences
   ...
   Entry/exit: SURF_FEEDBACK_OPERATIONS -> Saved preferences immediately affect visible product state or show a clear confirmation.
...
5. SURF_EMPTY_AND_ERROR_RECOVERY - Empty and Error Recovery
   ...
   Entry/exit: SURF_FEEDBACK_OPERATIONS, SURF_INSIGHTS -> Recovery returns to the active Feedback workflow and preserves unrelated state.
